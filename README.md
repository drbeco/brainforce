...
| flag  | description  |
|---|---|
| -y | Print the syntax tree of the input_file. This is the structure of the syntax parse tree made by bison/yacc |
...
| -h | Print this help. -v -h for advanced help. -vv for expert help. -vv... for debug information. |
...
| | -z n,m | n=minimum (>= 0), m=maximum (<= UINTMAX) and n<m |
...
| |        | -   if m<=UCHARMAX set -t uchar |
| |        | -   if UCHARMAX<m<=USHORTMAX set -t ushort |
| |        | -   if m>USHORTMAX set -t uint |
|---|---|---|
| | -z bit | Same as -z 0,1
| | -z print | Same as -z 32,126 (ASCII printable). Nice print if used with -p uchar or -p both, and -r on |
| | -z number | Same as -z 48,57  (ASCII '0' to '9') |
| | -z letter | Same as -z 65,90  (ASCII 'A' to 'Z') |
| | [-z uchar] | (default) Same as -z 0,UCHARMAX |
| |-z ushort | Same as -z 0,USHORTMAX. Set -t ushort, unless told otherwise explicitaly by -t |
| | -z uint |Same as -z 0,UINTMAX (\~ 4 billions). Set -t uint, unless told otherwise explicitaly by -t |
| | | If -r on or -r off is used, it is useful to read warnings given by
verbosity level 1 or more (-v) |
| -t type | | Type of array. The type of array must hold the range of values set by -z
options. |
...
| -a n | | (default) -a USHORTMAX+1. Array size (quantity of cells), n=[1,USHORTMAX+1] |
| | | The pointer is type ushort unless: |
| | | -a UCHARMAX+1 or less is used, and -w none the pointer is made uchar. |
| | | -a UCHARMAX or less is used, and -w on/off, the pointer is made uchar. |
| | | See -w to understand the proprierties of sizes UCHARMAX+1 and USHORTMAX+1 |
...
| | -# 1 | Debug mode off, ignore all \#'s in the resulted binary, but show it as
...
| -r wrap | | Set option for wrapping around cell values when they reach a limit. Wrap can be: |
| | -r on | Causes wrap around if cell overflow or underflow. |
| |-r off | Causes error if cell overflow or underflow. |
| | [-r none] | (default) Do not check bounds. Be careful: bounds depends on type size of the target machine. |
...
| | | Example: |
| | | -   To have -z n,UCHARMAX  and -r on/off, set -t ushort |
| | | -   To have -z n,USHORTMAX -r on/off, set -t uint |
| | | -   Don't worry about -z uint, it's guaranteed to work, because UINTMAX is one less than the full type. |
| -w wrap | | Set option for wrapping around the pointer when it reachs any limit. |
| | | If -w is not given, [-w none] is the default. |
| | | Pointer maximum limit is (n-1) set by -a n. Minimum is allways cell number 0. |
| | | Wrap can be: |
| | -w on | Causes wrap around if pointer overflow or underflow |
...
| | | With -a n and: |
| | | - 1\<=n\<=UCHARMAX, pointer is type uchar. |
| | | - UCHARMAX+1\<n\<=USHORTMAX+1, pointer is type ushort. |
| | | - Pointers cannot be uint. |
| | -w off | Causes (controled) error if pointer overflow or underflow. |
...
| | | - UCHARMAX+1\<n\<=USHORTMAX+1, pointer is type ushort. |
| | | - Pointers cannot be uint. |
| | [-w none] | (default) Do not check for limis. Creates simpler code |
| | | WARNING! If -a is not UCHARMAX+1 or USHORTMA+1, the pointer can raise real Segmentation Fault errors depending on the BrainForce user source code! |
| | | How nice: If -a is set to those magic numbers, overflow/underflow just wraps around like -w on |
| | -w expand | (not implemented) If pointer reaches the right limit, redimension |
| | | array, till it gets the maximum size (or the memory is over). |
| | | When it cannot allocates more memory, it becomes -w off. |
| -p style | | Set how to print and read. Style can be: |
...
mode on -- see -i on) |
|-i [opt] | Iterative mode, set input/output from/to file or iterativelly (keyboard/monitor). Options are: |
| | -i on | Read/write from keyboard/screen. Uses ungetc(), or simplified scanf("%d"), depending on -p |
| | -i off | (default) read/write from/to file. Its necessary to use redirection symbols '\<' and '\>' from SO command line. Match getc()/putc() or scanf()/printf() (see -p) |
| | -i switch | The compiled program inherit '-i' switch to chose from command line
before each execution. Set argc/argv to read the switch that changes mode. So after compiling a test.bf, lets say: |
...
functions. |
| | | Also set -z ushort, -t ushort, -a USHORTMAX+1, debug mode off -\# 0, -r none, -w none, -p uchar, -i off. |
